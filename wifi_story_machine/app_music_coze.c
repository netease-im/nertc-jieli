...
#if 1//(!defined CONFIG_DUI_SDK_ENABLE) && (!defined CONFIG_TVS_SDK_ENABLE)

#ifdef CONFIG_NET_ENABLE
#include "network_download/net_download.h"
#include "ai_sdk/ai_coze_server.h"
#include "ai_sdk/nertc_protocol.h"
...
//luy
#include "ai_sdk/ai_coze_server.h"
#include "ai_sdk/ai_device_manager.h"
//end_luy
...
{
    app_music_play_mix_file(CONFIG_VOICE_PROMPT_FILE_PATH"ring.mp3");
}

void my_set_prompt_poweron_close(int id)
{
    u8 close_prompt = 1;
    syscfg_write(id, &close_prompt, sizeof(close_prompt));
}
// 0 提示音播放， 1 关闭提示音
u8 my_get_close_prompt_flag(int id)
{
    u8 close_prompt = 0;
    if(syscfg_read(id, &close_prompt, sizeof(close_prompt)) < 0) {
        close_prompt = 0;
    }
    printf("my_get_close_prompt_flag = %d, %d\n", close_prompt, id);
        

    u8 clear = 0;
    syscfg_write(id, &clear, sizeof(clear));
...
static u8 open_wifi_connect_tone = 1;
void app_music_play_connect_success(void)
{
    if(open_wifi_connect_tone)
    {

        u8 close_prompt = my_get_close_prompt_flag(CFG_USER_CLOSE_NETWORK_PROMPT);
        printf("close_prompt = %d\n", close_prompt);
        if(!close_prompt)
            app_music_play_voice_prompt("NetCfgSucc.mp3", __this->dec_ops->dec_breakpoint); //语音提示：网络连接成功
        open_wifi_connect_tone = 0;
    }
}
// static u8 update_status = 0;
// void app_music_play_update_state(u8 state)
// {
//     struct net_event net = {0};
//     update_status = state;
//     if(state == 1)
//         net.event = NET_OTA_START;
//     else if(state == 2)
//         net.event = NET_OTA_ING;
//     else
//     {
//         net.event = NET_OTA_FINISH;
//         update_status = 0; 
//     }
//     net.arg = "net";
...
// void app_music_play_connect_fail(void)
// {
//     app_music_play_voice_prompt("NetCfgFail.mp3", NULL); 
// }

// void app_music_play_connect_succ(void)
// {
//     app_music_play_voice_prompt("SerConnect.mp3", NULL); 
// }
...
static int app_music_ai_listen_start(void);
...
static int app_music_ai_listen_start(void){
    /* 注意: 不再直接调用 nertc_protocol_open_audio_channel()
     * 统一由 AI_EVENT_RTC_ONOFF 事件处理器根据状态机控制 */
#if GIZWITS_RTC_ENABLE
    printf("------aaaaaaa----------app_music_ai_listen_start");
    ai_send_event_to_sdk(AI_EVENT_RTC_ONOFF, 0);
#else
    ai_send_event_to_sdk(AI_EVENT_RECORD_START,0);
#endif
//     int err;
//     union ai_coze_req req = {0};
...
//     //
//     if (__this->listening != LISTEN_STATE_STOP) {   //如果返回，说明是在监听状态
//         //printf("mic listening\n");
//         return -1;
//     }

//     if (!__this->ai_server->ai_connected || !__this->net_connected) {
...
//     //app_ai_sdk_event_cb(AI_EVENT_RECORD_BREAK, NULL);   //打断录音
//     //app_music_play_voice_prompt("rec.mp3", NULL);       //播放提示应
...
//     req.evt.arg = voice_mode | ((CONFIG_AUDIO_ENC_SAMPLE_SOURCE + 4) << 3);
// #else
// #ifdef CONFIG_ALL_ADC_CHANNEL_OPEN_ENABLE
//     req.evt.arg = voice_mode | ((CONFIG_ASR_CLOUD_ADC_CHANNEL + 1) << 3);
// #else
//     req.evt.arg = voice_mode;
// #endif
// #endif
//     req.evt.ai_name = AI_COZE_SDK_NAME;
//     req.evt.event = AI_LISTEN_START;
    
//     err = ai_server_request(__this->ai_server, AI_REQ_EVENT, &req);
//     if (err == 0) {
//         __this->listening = LISTEN_STATE_START;     //设置开始录音状态
//         os_time_dly(30);
//     }
    return 0;
}
...
static void app_music_ai_listen_stop(void){
    ai_send_event_to_sdk(AI_EVENT_RECORD_STOP,0);
    // union ai_coze_req req = {0};
    // printf("listening: %d", __this->listening);
...
static void app_music_event_net_connected(){
    ...
    //luy
    __this->ai_server = ai_coze_server_open(AI_COZE_SDK_NAME, NULL);
    if (__this->ai_server) {
         ai_coze_server_register_event_handler(__this->ai_server, NULL, ai_server_event_handler);
...
{
     // 如果配置了蓝牙且支持 Wi-Fi 配置共存，并且当前模式不是蓝牙音乐模式
#if defined CONFIG_BT_ENABLE && BT_SUPPORT_WIFI_CFG_COEXISTENCE
    if (__this->mode != BT_MUSIC_MODE)
#endif
    {
...
// 如果配置了分配 MAC 地址功能，取消服务器分配 MAC 地址的操作
    ...
    printf("----------------------------------------------------------------->:%d",is_in_config_network_state());
    //如果设备未处于配置网络状态
    if (!is_in_config_network_state()) {
        // 暂停自动关机
        ...
        // 如果 AI 服务器存在
        if (__this->ai_server) {
            ai_coze_server_close(__this->ai_server);
    ...
    {
        // 如果配置了蓝牙且支持 Wi-Fi 配置共存，并且蓝牙音乐正在播放
#if defined CONFIG_BT_ENABLE && BT_SUPPORT_WIFI_CFG_COEXISTENCE
        if (get_bt_music_dec_ops()->dec_status(0) == AUDIO_DEC_START) {
...
#if (GIZWITS_RTC_ENABLE==0)
    case KEY_ENC:
        //printf("app_music_key_hold -> KEY_ENC\n");
...
static int app_music_key_event_handler(struct key_event *key){
    int ret = false;
    //printf("value:%d action:%d", key->value, key->action);
    // if(update_status){
    //     //app_music_play_update_state(2);
    //     return 0;
    // }
    ...
    case KEY_EVENT_CLICK:
        #ifdef CONFIG_NET_ENABLE
            #if (GIZWITS_RTC_ENABLE==1)
            printf("key:%d",key->value);
            switch(key->value){
                    case KEY_ENC:
                        app_music_ai_listen_start();
                    break;
...
static int app_music_net_event_handler(struct net_event *event)
{
    if (!ASCII_StrCmp(event->arg, "net", 4)) {
        ...
        printf("NET_CONNECT_TIMEOUT_NOT_FOUND_SSID\n");
            if (__this->mode == NET_MUSIC_MODE) {
                app_music_play_voice_prompt("NetCfgFail.mp3", NULL);    //语音提示：网络连接失败，请重新帮我联网吧
#if defined CONFIG_TURING_SDK_ENABLE && BT_NET_CFG_TURING_EN
                turing_ble_cfg_net_result_notify(0, NULL, NULL, NULL);
#elif BT_NET_CFG_EN
                printf("-------------------------------------ble_cfg_net_result_notify:%d",event->event);
        ...
        case NET_EVENT_SMP_CFG_FIRST:
            if (__this->mode == NET_MUSIC_MODE) {
                app_music_play_voice_prompt("NetEmpty.mp3", NULL);  //语音提示：请根据语音说明，给我配置网络哦
        ...
        case NET_EVENT_SMP_CFG_FINISH:
            printf("------------------------------------rev NET_EVENT_SMP_CFG_FINISH\n");
...
#endif
                {
                    app_music_play_voice_prompt("SsidRecv.mp3", NULL);  //语音提示：已收到配网信息，稍等一下哦
                }
                printf(";;;;;;;;;;;;;;;;;;12;;;;;;;;;;;;;");
                //config_network_stop();
...
#endif
                sys_power_auto_shutdown_resume();
#if TCFG_USER_BLE_ENABLE && (BT_NET_CFG_EN || BT_NET_CFG_DUI_EN || BT_NET_CFG_GIZWITS_EN) && CONFIG_POWER_ON_ENABLE_BLE || (defined PAYMENT_AUDIO_SDK_ENABLE)
            } else {
#if defined CONFIG_BT_ENABLE && BT_SUPPORT_WIFI_CFG_COEXISTENCE
                if (get_bt_music_dec_ops()->dec_status(0) == AUDIO_DEC_START) {
                    app_music_play_voice_prompt("SsidRecv.mp3", __this->dec_ops->dec_breakpoint);
        ...
        case NET_EVENT_CONNECTED:
            ...
            // @TODO: 启动 MQTT 连接
            // 测试期间先初始化基于 WebSocket 的 Chat 模式
            // giz_chat_init();
        ...
        case NET_EVENT_DISCONNECTED:
            ...
            if (__this->net_connected && !is_in_config_network_state() && !__this->reconnecting && __this->mode == NET_MUSIC_MODE) {
                app_music_play_voice_prompt("NetDisc.mp3", NULL);   //语音提示：网络已断开
            }
            ...
            //MY_DEBUG
            open_wifi_connect_tone = 1;

            // printf("my cpu_reset!!\n");
            // u8 reset_src = 1;
            // syscfg_write(CFG_USER_RESET_SRC, &reset_src, sizeof(reset_src));
            // os_time_dly(300);
            // cpu_reset();
...
//蓝牙事件预处理函数，用于在处理蓝牙事件之前对事件进行初步处理
