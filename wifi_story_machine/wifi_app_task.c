...
#define INIT_PWD  ""
#else
/* 硬编码 WiFi 配置，开机自动连接，无需蓝牙配网 */
#define INIT_MODE STA_MODE
#define FORCE_DEFAULT_MODE 1 //强制使用默认配置
#define INIT_SSID "fishnet"
...
void wifi_and_network_on(void)
{
    ...
    printf("[MEM_TRACE] === before wifi_on ===\n");
    malloc_stats();
    wifi_on();
    printf("[MEM_TRACE] === after wifi_on ===\n");
    malloc_stats();
    net_app_init();
    printf("[MEM_TRACE] === after net_app_init ===\n");
    malloc_stats();
}
...
static int wireless_net_init(void)   //主要是create wifi 线程的
{
    ...
    printf("[MEM_TRACE] === wireless_net_init entry ===\n");
    malloc_stats();
